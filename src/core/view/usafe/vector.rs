...
use super::{StorageViewUsf, VectorViewUsf};
...
impl<'a, T: WithDType> VectorViewUsf<'a, T> {
    ...
    pub unsafe fn drop(&self, size: usize) -> Result<Self> {
        ...
        Ok(Self {
            storage: StorageViewUsf( unsafe { self.storage.add(self.storage_index(size)) } ),
            len: self.len - size,
            stride: self.stride,
            _marker: PhantomData,
        })
    ...
    pub unsafe fn slice<R: Into<Range>>(&self, range: R) -> Result<Self> {
        ...
        let storage = StorageViewUsf( unsafe { self.storage.add(self.storage_index(start)) } );
...
impl<'a, T: NumDType> VectorViewUsf<'a, T> {
    ...
    pub unsafe fn mul_assign(&mut self, mul: T) {
        for i in 0..self.len {
